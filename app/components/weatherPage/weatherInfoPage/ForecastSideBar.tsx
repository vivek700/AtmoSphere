...
const ForecastSideBar = ({ daily }: { daily: DailyWeatherData[] }) => {
    ...
    useEffect(() => {
        ...
        if (container) {
            container.scrollIntoView({ behavior: 'instant', inline: 'start', block: 'nearest' });
    ...
    const infoElement = daily?.map((info: DailyWeatherData) => (
        <menu onClick={(e) => handleClick(info.id)} key={info?.id} className='flex rounded h-11 border-radius cursor-pointer hover:bg-neutral-800 text-sm md:text-base '>
            <li className='py-2 flex-1 pl-2'>{info?.time}</li>
            <li className='py-2 flex-1'>{`${info?.temp.max} / ${info?.temp.min}`}&#8451;</li>
            <li className=' py-1 pr-2 max-w-20 text-center text-xs'>{info?.description}</li>
        </menu>
    ))
    ...
    const topbarElement = daily?.map((info: DailyWeatherData) => (
        <li
            ...
            key={info?.id} onClick={(e) => handleClick(info?.id)} className={`${checkId === info?.id ? "text-red-500" : ""} flex-1 mx-2 cursor-pointer`} >{info?.time}</li>
    ))
            ...
            return (
                <section key={info?.id} className="text-sm md:text-base">
                    <section className="py-6">
                        <p className="text-lg text-neutral-300">{capitalizeFirstLetter(info?.description)}.</p>
                    ...
                    <section className={`pt-6 overflow-x-auto  ${styles.scroll_div}`}>
                        <p className="pl-20">
                            <span className="px-4">Morning</span>
                            <span className="px-4">Afternoon</span>
                            <span className="px-3">Evening</span>
                            <span className="px-3">Night</span>
                        </p>
                        < p >
                            <span className="text-xs">TEMPERATURE</span>
                            <span className="px-6 md:px-6">{info?.temp.morn}&#8451;</span>
                            <span className="px-8 md:px-10">{info?.temp.day}&#8451;</span>
                            <span className="px-7 md:pl-6">{info?.temp.eve}&#8451;</span>
                            <span className="pl-2">{info?.temp.night}&#8451;</span>
                        </p >
                        <p className="whitespace-nowrap">
                            <span className="text-xs">FEELS LIKE</span>
                            <span className="pl-12 pr-6">{info?.feels_like.morn}&#8451;</span>
                            <span className="px-8 md:px-10">{info?.feels_like.day}&#8451;</span>
                            <span className="px-7 md:pl-6">{info?.feels_like.eve}&#8451;</span>
                            <span className="pl-[0.5rem]">{info?.feels_like.night}&#8451;</span>
                        </p>
                    </section >
    ...
    return (
        <>
            <p className='pb-4 pt-5 md:pt-0 text-2xl overflow-x-auto'>8-day forecast</p>
            {pressed ?
                <div  >
                    <menu ref={navRef}
